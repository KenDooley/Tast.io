...
DICTIONARY = corpora.Dictionary.load('restaurants.dict')
CORPUS = corpora.MmCorpus('corpus.mm')
TFIDF_MODEL = models.TfidfModel.load('model.tfidf')
LSI_MODEL = models.LsiModel.load('model.lsi')
INDEX = similarities.MatrixSimilarity.load('restaurant.index')

#### Pickles
with open('idx_name.obj', 'r') as f:
...
if __name__ == '__main__':
    ...
    for idx, sim in enumerate(sims):
        if IDX_NAME[sim[0]]['location'] == state:
            output.append((sim[1], IDX_NAME[sim[0]]['name']))
            if len(output) == 30:
                break
