...
CORPUS = corpora.MmCorpus('corpus_bigrams.mm')
TFIDF_MODEL = models.TfidfModel.load('model_bigrams.tfidf')
...
def serialize_corpus():
    ...
    file1 = open('name_idx.obj', 'w') 
    file2 = open('idx_name.obj', 'w') 
...
def initialize_tfidf():
    ...
    tfidf.save('model_bigrams.tfidf')

def initialize_lsi():
    corpus_tfidf = TFIDF_MODEL[CORPUS]
    lsi = models.LsiModel(corpus_tfidf, id2word=DICTIONARY, num_topics=FACTORS)
    lsi.save('model.lsi')

def initialize_lda_bigrams():
    lda = models.LdaMulticore(TFIDF_MODEL[CORPUS], id2word=DICTIONARY, chunksize=1000, num_topics=FACTORS, workers=2)
    #corpus_tfidf = TFIDF_MODEL[CORPUS]
    #lsi.add_documents(corpus_tfidf, chunksize=10)
    lda.save('model_bigrams.lda')

def initialize_sim_matrix():
    corpus_tfidf = TFIDF_MODEL[CORPUS]
    index = similarities.MatrixSimilarity(LSI_MODEL[corpus_tfidf])
    index.save('restaurant.index')

if __name__ == '__main__':

    initialize_lda_bigrams()
